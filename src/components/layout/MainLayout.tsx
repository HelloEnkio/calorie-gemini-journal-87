  ...
  return <div className="main-layout">
      <div className="fixed top-0 left-0 right-0 h-16 backdrop-blur-md bg-background/80 px-4 z-10 flex items-center justify-between shadow-sm">
        <div className="flex items-center">
          <span className="font-medium text-lg">ZeaLog</span>
          {subscriptionPlan === "premium" && <span className="ml-2 bg-primary/10 text-primary text-xs font-medium px-3 py-1 rounded-full">
              Premium
            </span>}
        </div>
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="w-56 rounded-xl">
              <div className="flex flex-col space-y-1 p-2">
                ...
                {isLoggedIn && subscriptionPlan && <p className="text-xs text-muted-foreground">
      ...
      <button id="signup-button" onClick={() => document.getElementById("register-trigger")?.click()} className="hidden" />
    </div>;
};
