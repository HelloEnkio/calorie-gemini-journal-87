...
const CreateFoodModal = ({ isOpen, onClose, initialFoodName = "", onCreated }: CreateFoodModalProps) => {
  ...
  const calculateRecipeMacros = () => {
    ...
    ingredients.forEach(ingredient => {
      ...
      if (foodItem) {
        const ratio = ingredient.quantity / (foodItem.weight || 100);
                  ...
                  {ingredients.map((ing, index) => (
                    <div key={index} className="flex items-center justify-between py-2 border-b">
                      <div>
                        ...
                        <span className="text-sm text-muted-foreground ml-2">{ing.quantity}g</span>
                      </div>
                      <Button
                        variant="ghost"
                        size="icon"
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    ...
                    <div>
                      ...
                      <span className="font-medium ml-1">
                        {ingredients.reduce((sum, ing) => sum + ing.quantity, 0)}g
                      </span>
                    </div>
                  </div>
                </div>
